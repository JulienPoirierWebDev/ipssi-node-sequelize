...
const createOneCharacter = async (request, response) => {
  ...
  if (
    ...
    return response.status(400).json({
      message:
        'Le corps de la requête doit contenir name et description, en string',
      error: true,
    });
  }
  ...
  try {
    ...
    response
      .status(201)
      .json({ message: 'Le personnage a été creé', character: newCharacter });
  } catch (error) {
    const sendingError = process.env.ENV === 'DEV' ? error : true;
    response.status(500).json({
      message: "Le personnage n'a pas pu être crée a cause du serveur",
      error: sendingError,
    });
  }
};
...
const getAllCharacter = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .json({ message: 'Erreur du server', error: sendingError });
  }
};
  ...
  try {
    ...
    if (!character) {
      return response.status(404).json({
        message: "Il n'y a pas de personnage avec cet ID",
        error: true,
      });
    }
...
const updateOneCharacterById = async (request, response) => {
  ...
  try {
    ...
    response
      .status(201)
      .json({ message: 'Personnage modifié avec succès', result: character });
  } catch (error) {
    const sendingError = process.env.ENV === 'DEV' ? error : true;
